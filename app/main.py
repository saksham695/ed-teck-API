...
class Store:
    ...
    def _seed(self) -> None:
        ...
        for course_data in DUMMY_DATA["courses"]:
            course = Course(**course_data)
            self.courses[course.id] = course

        for enrollment_data in DUMMY_DATA["enrollments"]:
            enrollment = Enrollment(**enrollment_data)
            self.enrollments[enrollment.id] = enrollment

        for progress_data in DUMMY_DATA["progress"]:
            progress = Progress(**progress_data)
            self.progress[progress.enrollment_id] = progress

        # Keep auto-increment counters ahead of seeded records.
        self._next_ids["course"] = max(self.courses.keys(), default=0) + 1
        module_ids = [m.id for c in self.courses.values() for m in c.modules]
        self._next_ids["module"] = (max(module_ids) if module_ids else 0) + 1
        lecture_ids = [l.id for c in self.courses.values() for m in c.modules for l in m.lectures]
        self._next_ids["lecture"] = (max(lecture_ids) if lecture_ids else 0) + 1
        assignment_ids = [a.id for c in self.courses.values() for m in c.modules for a in m.assignments]
        self._next_ids["assignment"] = (max(assignment_ids) if assignment_ids else 0) + 1
        question_ids = [q.id for c in self.courses.values() for m in c.modules for a in m.assignments for q in a.questions]
        self._next_ids["question"] = (max(question_ids) if question_ids else 0) + 1
        self._next_ids["enrollment"] = max(self.enrollments.keys(), default=0) + 1
